...
Accepted work runs inside the Durable Object itself, on the Agents SDK's durable [Tasks](https://github.com/cloudflare/agents/blob/main/docs/agents/tasks.md) capability. Admission ensures a `flue:drive@v1` run, whose one bounded pass claims the queued submission and starts a `flue:attempt@v1` run for it; that run's handler awaits the full response — every model turn and tool call — and settles the submission. Admission always returns first; the response begins in the same invocation when the object is warm, or on the next alarm tick otherwise. The SDK owns the durable wake while a run is held (a claim backstop that survives eviction), replays the attempt run on a fresh isolate after an interruption, and enforces the submission's [durability timeout](/docs/guide/durability/#retry-budget-and-timeout) as the run's deadline. Three consequences:
...
- **Scheduled callbacks share the alarm with running responses.** The SDK drives due work from one alarm loop and hands a long attempt off after a short dispatch budget, so `schedule()`/`scheduleEvery()` callbacks that come due mid-response still fire — delivery is durable, and timeliness is best-effort while the agent is busy. `queue()` is not alarm-driven and is unaffected. Steering is also unaffected: a message that arrives mid-response still joins it at the next turn boundary.
