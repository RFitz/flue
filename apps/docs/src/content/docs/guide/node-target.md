...
Several processes can share one durable database without corrupting a conversation: each claimed attempt fences out earlier writers, and a process stops an attempt whose lease another process reclaimed. On the Postgres, MySQL, and libSQL adapters a conversation also has a named owner: the process that claims its work keeps claiming it while work keeps flowing, and releases it after 30 seconds without any (`FLUE_OWNER_IDLE_TTL_MS` overrides the window). Any process can still accept a conversation's input; only the owner runs it. Once the owner goes idle, the next turn may run in another process, which drops process-local warm state such as MCP connections and sandboxes. [Durability](/docs/guide/durability/#nodejs-recovery) covers what leases cannot bound. In-process [schedules](/docs/guide/schedules/) fire in every replica.
