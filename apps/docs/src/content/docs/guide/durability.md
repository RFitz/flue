...
- **Replicas stay correct over a shared database.** Every process can admit, claim, and recover a conversation's work without corrupting it. Each claimed attempt takes the conversation's stream producer, which fences out every earlier writer in any process. The lease heartbeat stops an attempt whose lease another process reclaimed, ending its model and tool calls at their next abort check. Correctness does not make placement free: a turn that runs in a different process than the last one loses that process's warm state (MCP connections, sandboxes), and a stalled attempt's side effects are bounded only as described below.
- **A busy conversation stays on one process.** On the Postgres, MySQL, and libSQL adapters, the process that claims a conversation's work becomes its owner, and only the owner claims its later work while the conversation is busy. The owner lets go after an idle window, 30 seconds by default and set per deployment with `FLUE_OWNER_IDLE_TTL_MS`, so a conversation that is quiet for longer can resume on any process. Only claiming and running work keeps an owner; reading history or streaming a conversation does not. A graceful shutdown releases ownership at once; a crashed owner's conversations move after the window lapses.
